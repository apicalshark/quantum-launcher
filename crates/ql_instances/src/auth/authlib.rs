use ql_core::{file_utils, pt, DownloadFileError, IntoIoError, LAUNCHER_DATA_DIR};
...
/// `-javaagent:YOUR_LAUNCHER_DATA_DIR/downloads/authlib_injector.jar=ely.by`
pub async fn get_authlib_injector(url: &str) -> Result<String, DownloadFileError> {
    ...
    let dir = LAUNCHER_DATA_DIR.join("downloads");
